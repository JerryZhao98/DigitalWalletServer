...
public class CustomerData {
	...
	public int updateCust(Customer cust) {
		...
		try {
			 ...
			 */
			rs = stmt.executeUpdate("EXEC UpdateCustomer " + custID + ",'" + fName + "','" + lName + "','" + email
					+ "'," + phoneNum + "," + addrID + ",'" + password + "';");
	...
	public int saveCust(Customer cust) {
		...
		ResultSet tempRS = getByEmail(email);
		try {
			if (!tempRS.next()) {

				try {
					rs = stmt.executeUpdate("EXEC AddCustomer '" + fName + "','" + lName + "','" + email + "',"
							+ phoneNum + "," + addrID + ",'" + password + "';");
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public ResultSet login(String email, String password) {
		...
		try {
			rs = stmt.executeQuery(
					"select * from tblCustomer where fldEmail='" + email + "' and fldPassword='" + password + "';");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
